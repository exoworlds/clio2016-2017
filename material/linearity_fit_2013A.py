...
## Values from IDL
ints_uniq = np.array([43.0000,100.000,150.000,200.000,300.000,400.000,500.000,750.000,1000.00,1250.00,1500.00,1750.00,2000.00,2250.00,2500.00,2750.00,3000.00,3250.00,3500.00,3750.00,4000.00])
...
#   8.5524125e-07
counts_true = np.array([5406.5706,6211.3086,6917.2191,7623.1296,9034.9507,10446.772,11858.593,15388.146,18917.698,22447.251,25976.804,29506.356,33035.909,36565.462,40095.014,43624.567,47154.120,50683.673,54213.225,57742.778,61272.331])


## Find fits
idx0 = np.where((counts_true > 9e3) & (counts_true < 1.6e4))
coeff1 = np.polyfit(counts_true[idx0], counts_mean[idx0], 1)
num = 0
end = 60000
...
idx = np.where((counts_true > 1e3) & (counts_true < 4.5e4))
coeff2 = np.polyfit(counts_true[idx], counts_mean[idx], 2)
yarr2 = []
for i in xarr:
	yarr2.append(coeff2[0]*xarr[i]*xarr[i] + coeff2[1]*xarr[i] + coeff2[2])


coeff3 = np.polyfit(counts_true[idx], counts_mean[idx], 3)
yarr3 = []
for i in xarr:
	yarr3.append(coeff3[0]*xarr[i]*xarr[i]*xarr[i] + coeff3[1]*xarr[i]*xarr[i] + coeff3[2]*xarr[i] + coeff3[3])


coeff4 = np.polyfit(counts_true[idx], counts_mean[idx], 4)
yarr4 = []
for i in xarr:
...
coeff5 = np.polyfit(counts_true[idx], counts_mean[idx], 5)
yarr5 = []
for i in xarr:
...
## Plot it
plt.scatter(counts_true,((counts_mean-counts_true)/counts_true))
...
## Plot it
plt.scatter(counts_true,counts_mean,color='blue')
plt.scatter(counts_true,counts_corrected,color='red')
plt.plot(xarr,yarr1,color='black')
plt.xlabel('True counts')
plt.ylabel('Measured counts')
#plt.title('True counts based on linear fit of data between 500ms and 1500 ms vs. departure from linearity',fontsize = 9)
#plt.text(40000,-.08,'Desired relationship: Black, 2nd order polynomial: Red, 3rd order polynomial: Blue', fontsize=7, horizontalalignment='center')
...
red_patch = mpatches.Patch(color='red', label='Corrected')
plt.legend(handles=[blue_patch,red_patch],bbox_to_anchor=(1, .2))
plt.show()
