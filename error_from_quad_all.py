...
coefficients2 = numpy.polyfit(true_counts[15:80],counts[15:80],1)
polynomial_lin = numpy.poly1d(coefficients2)
xpoints = numpy.linspace(7500,70000,500)
...
error_init = []
while number < end_number:
	error_init.append((true_counts[number]-counts[number])/true_counts[number])
	number+= 1

# Feed data into pyplot.
plt.plot(true_counts,error_init,'k')
plt.plot(xpoints,((xpoints-polynomial_lin(xpoints))/xpoints),'r')
